...
void UIInputText::DeleteBackward()
{
	...
	mInputTextCharacterIndex = leftStr.length();
...
void UIInputText::InsertArrow(int dirType)
{
	...
	else if (1 == dirType)
	{
		...
		if (mInputTextCharacterIndex > (int)mText.length())
			mInputTextCharacterIndex = mText.length();
	}
...
//----------------------------------------------------------------------------
bool UIInputText::AttachWithIME(const APoint &worldPos)
{
	if (IMEDelegate::AttachWithIME(worldPos))
	{
