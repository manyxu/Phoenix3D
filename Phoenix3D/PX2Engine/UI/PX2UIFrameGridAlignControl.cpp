										   ...
										   ChildAlignmentType childAlignmentTypeType)
{
	...
	for (int i = 0; i < (int)frames.size(); i++)
	{
		...
		if (CAT_UPPER_LEFT == mChildAlignmentType)
		{
			...
			anchorParamHor[0] = halfItemSize[0] + (mSpacing[0] + cellSize.Width)*indexX + mBorder[0];
			anchorParamVer[0] = -(halfItemSize[1] + (mSpacing[1] + cellSize.Height)*indexZ) - mBorder[3];
		}
		else if (CAT_UPPER_CENTER == mChildAlignmentType)
		{
			...
			anchorParamHor[0] = -allCotWidth / 2.0f + halfItemSize[0] + (mSpacing[0] + cellSize.Width)*indexX;
			anchorParamVer[0] = -(halfItemSize[1] + (mSpacing[1] + cellSize.Height)*indexZ) - mBorder[3];
		}
