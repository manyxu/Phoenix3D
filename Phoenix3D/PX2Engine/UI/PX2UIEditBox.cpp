...
//----------------------------------------------------------------------------
void UIEditBox::OnAttachWithIME(const APoint &worldPos)
{
	PX2_UNUSED(worldPos);
...
//----------------------------------------------------------------------------
void UIEditBox::OnWidgetPicked(const UIInputData &data)
{
	if (!IsEnable())
		return;
	...
	if (UIPT_PRESSED == data.PickType)
	{
		if (!mIsAttachedIME)
		{
			_AttachIME(data.WorldPos);
		}
		else
		{
			mAttachedOffsetX = mInputText->GetOffset()[0];
			mAttachedSelectCharaIndex = _CalAttachCharaIndex(data.WorldPos);
	...
	else if (UIPT_MOVED == data.PickType)
	{
		...
		{
			int charaIndex = _CalAttachCharaIndex(data.WorldPos);
...
//----------------------------------------------------------------------------
void UIEditBox::OnWidgetNotPicked(const UIInputData &data)
{
