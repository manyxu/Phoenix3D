...
void UICanvas::_AddInRangePickWidget(UIFrame *widget)
{
	...
	SetPickedSomething(true);
}
//----------------------------------------------------------------------------
void UICanvas::_ClearInRangePickWidget()
{
	mInRangeWidgets.clear();
	SetPickedSomething(false);
}
//----------------------------------------------------------------------------
...
void UICanvas::_DoPick(const UIInputData &inputData)
{
	...
	if (mRenderNode)
	{
		UIFrame *uiFrame = DynamicCast<UIFrame>(mRenderNode);
		if (uiFrame)
	...
	for (; it != mPickWidgets.end(); it++)
	{
		if (inRangeWidget != *it)
		{
			(*it)->OnWidgetNotPicked(inputData);
		}
	}
}
//----------------------------------------------------------------------------
