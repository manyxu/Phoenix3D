...
mChoose(-1)
{
	...
	mChooseList->LocalTransform.SetTranslateY(-2.0f);
...
//----------------------------------------------------------------------------
void UIComboBox::RemoveAllChooses()
{
	mChooseList->RemoveAllItems();
}
//----------------------------------------------------------------------------
void UIComboBox::AddChooseStr(const std::string &choose)
{
	mChooses.push_back(choose);
	UIItem *item = mChooseList->AddItem(choose);
	item->SetUserData("index", (int)(mChooses.size() - 1));
}
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
std::string UIComboBox::GetChooseStr(int i) const
{
...
void UIComboBox::Choose(int i)
{
	...
	text->SetText(GetChooseStr(mChoose));
...
void UIComboBox::OnChoosed()
{
	if (mUICallback)
	{
		mUICallback(this, UICT_COMBOBOX_CHOOSED);
	}
