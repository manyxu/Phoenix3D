...
void UIButton::OnDoublePressed()
{
	...
	if (mMemObject && mMemUICallback)
	{
		(mMemObject->*mMemUICallback)(this, UICT_DOUBLE_PRESSED);
	}

	std::vector<Visitor *>::iterator it = mVisitors.begin();
	for (; it != mVisitors.end(); it++)
	{
		(*it)->Visit(this, (int)UICT_DOUBLE_PRESSED);
	}

	if (!mUIScriptHandler.empty())
	{
		CallString(mUIScriptHandler.c_str(), "i", (int)UICT_DOUBLE_PRESSED);
	}
}
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
void UIButton::OnWidgetPicked(const UIInputData &inputData)
{
...
//----------------------------------------------------------------------------
void UIButton::OnWidgetNotPicked(const UIInputData &inputData)
{
	ButtonState state = GetButtonState();
