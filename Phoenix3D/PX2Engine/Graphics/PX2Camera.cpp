...
Vector2f Camera::WorldPos3DTo2D(const Rectf& viewPort,
	const APoint &worldPos, bool *isInBack)
{
...
//----------------------------------------------------------------------------
AVector Camera::ScreenVectorToCamera(const Sizef &screenSize, 
	const AVector &screenVec)
{
	...
	if (IsPerspective())
	{
		...
		float rMin = 0.0f;
		float rMax = 0.0f;
		GetFrustum(dMin, dMax, uMin, uMax, rMin, rMax);

		camVec = AVector(rMax*2.0f * screenVec.X() / screenSize.Width,
			screenVec.Y(), uMax*2.0f * screenVec.Z() / screenSize.Height);
	}
