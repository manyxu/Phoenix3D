...
{
	Project *proj = PX2_APP.TheProject;
...
void Project::SetUIFrame(UIFrame *ui)
{
	if (mUIFrame)
	{
		UIFrame *frameOrigin = mUIFrame;

		mUIFrame = 0;
		Event *ent = PX2_CREATEEVENTEX(ProjectES, CloseUI);
		ent->SetData<UIFrame*>(frameOrigin);
		PX2_EW.BroadcastingLocalEvent(ent);
	}
	...
	if (mUIFrame)
	{
		...
		ent->SetData<UIFrame*>((UIFrame*)mUIFrame);
		PX2_EW.BroadcastingLocalEvent(ent);
	}
}
//----------------------------------------------------------------------------
