...
//----------------------------------------------------------------------------
void EU_CanvasBP::DoExecute(Event *event)
{
...
void EU_CanvasBP::CreateEditMenuOnBPFile(const APoint &pos)
{
	...
	const std::vector<std::string> &events = PX2_BPM.GetEvents();
	...
	const std::vector<std::string> &params = PX2_BPM.GetParams();
	...
	const std::vector<std::string> &gfs = PX2_BPM.GetGFs();
	...
	const std::vector<std::string> &optionStrs = PX2_BPM.GetOptions();
	...
	const std::vector<std::string> &operatorStrs = PX2_BPM.GetOperators();
