...
PX2_IMPLEMENT_RTTI(PX2, EngineCanvas, EU_CanvasStage);
...
//----------------------------------------------------------------------------
void EU_CanvasStage::DoExecute(Event *event)
{
	...
	else if (SimuES::IsEqual(event, SimuES::NewScene))
	{
		...
		if (mGridCanvas)
			mGridCanvas->SetCamera(camera);

		if (mSceneNodeCtrlCanvas)
			mSceneNodeCtrlCanvas->SetCamera(camera);
	}
	else if (SimuES::IsEqual(event, SimuES::CloseScene))
	{
		mGridCanvas->SetCamera(0);
		mSceneNodeCtrlCanvas->SetCamera(0);
	}
...
void EU_CanvasStage::_CreateGridGeometry()
{
	...
	mGridNode->WorldTransformIsCurrent = true;

	mGridCanvas = new0 Canvas();
	mGridCanvas->SetBeforeDrawClear(false, true, false);
...
void EU_CanvasStage::_CreateNodeCtrl()
{
	...
	mSceneNodeCtrlCanvas->LocalTransform.SetTranslateY(-1.0f);
	mSceneNodeCtrlCanvas->SetBeforeDrawClear(false, true, false);
...
void EU_CanvasStage::OnLeftDown(const APoint &worldPos)
{
	...
	if (Edit::ET_SCENE == et)
	{
		if (mSceneNodeCtrl)
		{
			mSceneNodeCtrl->OnLeftDown(mSceneNodeCtrlCanvas, viewPortPos);
...
//----------------------------------------------------------------------------
void EU_CanvasStage::_ClickSelectPos(const APoint &viewPortPos)
{
...
void EU_CanvasStage::OnLeftUp(const APoint &worldPos)
{
	...
	{
		if (mSceneNodeCtrl)
			mSceneNodeCtrl->OnLeftUp(mSceneNodeCtrlCanvas, worldPos);
...
void EU_CanvasStage::OnMouseWheel(const APoint &worldPos, float delta)
{
	...
	if (mSceneNodeCtrl)
		mSceneNodeCtrl->OnMouseWheel(mSceneNodeCtrlCanvas, delta1);
}
//----------------------------------------------------------------------------
...
void EU_CanvasStage::OnMotion(const APoint &worldPos)
{
	...
	else
	{
		...
		{
			if (mSceneNodeCtrl)
				mSceneNodeCtrl->OnMotion(mIsLeftPressed, mSceneNodeCtrlCanvas, curViewPortPos, lastViewPortPos);
...
void EU_CanvasStage::CreateMenuOnSelect(const APoint &pos)
{
	...
	if (node)
	{
		...
		PX2EU_MAN.Menu_Edit_AddSubItem("Stage", "EditCreate", "SceneActor", PX2_LMVAL("SceneActor"));
		PX2EU_MAN.Menu_Edit_AddSubItem("Stage", "EditCreateSceneActor", "Geometry", PX2_LMVAL("Geometry"));
		PX2EU_MAN.Menu_Edit_AddItem("Stage", "EditCreateSceneActorGeometry", "Plane", PX2_LMVAL("Plane"), "n_CreateActor_Plane()");
		PX2EU_MAN.Menu_Edit_AddItem("Stage", "EditCreateSceneActorGeometry", "Box", PX2_LMVAL("Box"), "n_CreateActor_Box()");
		PX2EU_MAN.Menu_Edit_AddItem("Stage", "EditCreateSceneActorGeometry", "Sphere", PX2_LMVAL("Sphere"), "");

		PX2EU_MAN.Menu_Edit_AddItem("Stage", "EditCreateSceneActor", "Actor", "Actor", "n_CreateActor_Actor()");
		PX2EU_MAN.Menu_Edit_AddItem("Stage", "EditCreateSceneActor", "Character", "Character", "n_CreateActor_Character()");
		PX2EU_MAN.Menu_Edit_AddItem("Stage", "EditCreateSceneActor", "Camera", PX2_LMVAL("Camera"), "n_CreateActor_Camera()");
		PX2EU_MAN.Menu_Edit_AddItem("Stage", "EditCreateSceneActor", "Light", PX2_LMVAL("Light"), "n_CreateActor_Light()");
		PX2EU_MAN.Menu_Edit_AddItem("Stage", "EditCreateSceneActor", "Trigger", PX2_LMVAL("Trigger"), "n_CreateActor_Trigger()");
		PX2EU_MAN.Menu_Edit_AddItem("Stage", "EditCreateSceneActor", "Terrain", PX2_LMVAL("Terrain"), "n_CreateActor_Terrain()");
		PX2EU_MAN.Menu_Edit_AddItem("Stage", "EditCreateSceneActor", "Sky", PX2_LMVAL("Sky"), "n_CreateActor_Sky()");
