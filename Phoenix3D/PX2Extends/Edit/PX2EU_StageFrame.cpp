...
void EU_StageFrame::AddStage()
{
	...
	mCanvasStage->SetBeforeDrawClear(true, true, true);
	mCanvasStage->SetClearColor(Float4::MakeColor(0, 162, 232, 255));
	...
	mCanvasStageUI->SetName("EU_CanvasStageUI");
	AttachChild(mCanvasStageUI);
	mCanvasStageUI->LocalTransform.SetTranslateY(-2.0f);
	mCanvasStageUI->SetBeforeDrawClear(false, true, true);
	mCanvasStageUI->SetAnchorHor(0.0f, 1.0f);
	mCanvasStageUI->SetAnchorVer(0.0f, 1.0f);
...
{
	if (mCanvasStage)
		DetachChild(mCanvasStage);
	mCanvasStage = 0;

	if (mCanvasStageUI)
		DetachChild(mCanvasStageUI);
	mCanvasStageUI = 0;
}
//----------------------------------------------------------------------------
